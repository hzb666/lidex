...
test('generateProject scaffolds the default example site with package metadata', async () => {
  const { generateProject } = require('../../packages/create-lidex/src/index.js');
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'create-lidex-project-'));
  ...
  try {
    ...
    const config = fs.readFileSync(path.join(projectRoot, 'lidex.config.js'), 'utf8');
    ...
    assert.equal(packageJson.dependencies['@lidex/lidex'], '^0.1.0');
    assert.equal(packageJson.scripts.dev, 'lidex --root . --port 3001');
    assert.match(config, /slugField: '_slug_'/);
    assert.match(home, /title: Lidex/);
...
test('generateProject rejects existing target directories', async () => {
  const { generateProject } = require('../../packages/create-lidex/src/index.js');
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'create-lidex-existing-'));
  const projectRoot = path.join(tempRoot, 'my-lab-site');
...
test('runInteractiveCreate asks for project name and theme then scaffolds the site', async () => {
  const { runInteractiveCreate } = require('../../packages/create-lidex/src/index.js');
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'create-lidex-interactive-'));
  const prompts = [];
  const outputs = [];
  ...
  try {
    ...
    assert.equal(outputs.some((line) => line.includes('Lidex Project Scaffolder')), true);
