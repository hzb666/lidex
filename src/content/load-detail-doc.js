...
function loadDetailDoc(config, blockConfig, slug) {
  if (/[\\/]/.test(slug) || slug.includes('..')) {
    throw new LidexError(`Detail slug "${slug}" must not contain path separators or ".." segments`);
  }
  ...
  if (!isPathInside(contentRoot, detailPath)) {
    throw new LidexError(`Detail slug "${slug}" resolves outside contentDir`);
  }

  if (!fs.existsSync(detailPath)) {
    throw new LidexError(`Detail document not found for slug "${slug}" at ${detailPath}`);
  }
