...
lead: Template ownership should be obvious, because most custom Lydex work is really a template or theme change rather than an engine change.
...
When a project says it wants a new layout, the first question should be which layer actually changes. Is it the global shell, the list card, the detail route, or the query view. Lydex keeps those choices explicit so customization does not blur into engine code.
