...
const THEMES = [
  {
    key: 'default',
    label: 'Default',
    description: 'The bundled Lydex example site.',
...
function writeProjectPackageJson(projectRoot, projectName, lydexVersion) {
  const packageJson = {
    name: toPackageName(projectName),
    private: true,
    version: '0.1.0',
    scripts: {
      dev: 'lydex --root . --port 3001',
      build: 'lydex --build --root . --out ./dist',
    },
    dependencies: {
      '@lydex/lydex': `^${lydexVersion || '0.1.0'}`,
    },
  };
...
async function generateProject(options = {}) {
  ...
  writeProjectPackageJson(projectRoot, projectName, options.lydexVersion);
...
function writeBanner(writeLine, colorize) {
  writeLine('');
  writeLine(colorize('Lydex Project Scaffolder', ANSI.bold, ANSI.cyan));
  writeLine(colorize('Create a new Lydex site from the bundled example.', ANSI.dim));
  writeLine('');
}
...
async function runInteractiveCreate(options = {}) {
  ...
  try {
    ...
    const result = await generateProject({
      ...
      projectName,
      themeKey,
      lydexVersion: options.lydexVersion,
    });
