---
title: Queries
description: Query blocks make cross-page references declarative in Lidex.
...
The current engine supports `from`, `where`, `sort`, `limit`, and `offset`. A good mental model is this: page Markdown declares the query, Lidex reads the indexed dataset for that block type, the query executor filters and sorts it, and the query template decides the final presentation.

One of the most important habits is to keep the source page authoritative. News items still belong here. Their detail routes still open under `/queries/:slug`, but the `slug` itself is generated from `title`, and Lidex expects the body file under `content/news/<slug>.md` plus images under `assets/news/<slug>/`. The homepage or any other page simply asks for another view over the same indexed records.
...
category: Example
summary: The bundled starter was rewritten as a Lidex explanation site while keeping the same rendering primitives underneath.
:::
