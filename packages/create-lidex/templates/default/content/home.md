...
Treat this homepage as the start of the tutorial, not as a marketing mockup. Everything under the fold is powered by the real `example/lidex.config.js`, the real files inside `example/content/`, and the real templates under `example/templates/`.

The fastest learning path is `Concepts` -> `Design` -> `Blocks` -> `Details` -> `Queries` -> `Theme`. By the time you finish those pages, you should know which file to create, which config key to change, and which command to run for the most common Lidex workflows.

If you want to reproduce this site locally, run `npx lidex --root ./example --port 3001`. If you want static output, run `npx lidex --build --root ./example --out ./dist`. If you change any Markdown, template, or theme file after a static build, you build again. In server mode, the public routes update after a restart, and admin edits refresh the in-memory index immediately.
...
The point of Lidex is that you do not maintain one set of Markdown for content and another set of route code for presentation. A page can hold prose, blocks, and queries together, while the config, templates, and theme define how those declarations become a site.
