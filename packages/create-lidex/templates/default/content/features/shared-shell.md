...
lead: The shell defines the chrome once, then Lidex feeds each route its own header and body content.
...
Each page still gets its own header because Lidex passes frontmatter such as `title`, `eyebrow`, `lead`, `heroImage`, and `heroAlt` into the header builder. That is how the homepage can keep a full-screen visual treatment while the inner pages use a more compact intro block.
