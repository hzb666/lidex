...
why: Design changes stay isolated to the site using Lidex instead of leaking back into engine code.
...
When you customize Lidex, start by asking which layer really needs to change. If the route and data contract stay the same, you probably only need a template or theme override. That keeps customization local and keeps the engine reusable.
